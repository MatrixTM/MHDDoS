...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MHDDoS Attack GUI")
    ...
    def init_layer7_ui(self):
        ...
        self.force_stop_btn = QPushButton("Froce stop")
    ...
    def init_layer4_ui(self):
        ...
        self.force_stop_layer4_btn = QPushButton("Froce Stop")
    ...
    def start_layer4_attack(self):
        ...
        self.log_message(f"Thread: {threads}, Durate: {duration}s.")
    ...
    def start_real_attack(self, attack_type, method, target, threads, duration, 
        ...
        try:
            ...
            self.event.set()
            self.log_message("Starting Attack...")
            ...
            while time.time() < start_time + duration and not stop_event.is_set():
                stats = (f"Target: {target}\n"
                         ...
                         f"Send date: {Tools.humanbytes(int(BYTES_SEND))}\n"
                         f"Time: {int(time.time() - start_time)}秒")
            ...
            self.log_message("Attack already stopped.")
    ...
    def stop_attack(self):
        if self.attack_thread and self.attack_thread.isRunning():
            ...
            if not self.attack_thread.wait(2000):
                self.log_message("Normal stop timed out, trying to force stop...")
                    ...
                    if t.is_alive():
                        try:
                            ...
                            if thread_id:
                                ...
                                if res == 0:
                                    self.log_message(f"Cannot stop thread {thread_id}")
                                elif res != 1:
                                    ctypes.pythonapi.PyThreadState_SetAsyncExc(thread_id, None)
                        except Exception as e:
                            self.log_message(f"Failed to terminate thread: {str(e)}")
    
        self.log_message("All attack threads are stopped.")
        self.attack_finished()
        exit(0)
